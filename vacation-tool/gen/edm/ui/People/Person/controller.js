angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', '$http', 'messageHub', 'entityApi', 'Extensions', function ($scope, $http, messageHub, entityApi, Extensions) {
		...
		Extensions.get('dialogWindow', 'vacation-tool-custom-action').then(function (response) {
			$scope.pageActions = response.filter(e => e.perspective === "People" && e.view === "Person" && (e.type === "page" || e.type === undefined));
			$scope.entityActions = response.filter(e => e.perspective === "People" && e.view === "Person" && e.type === "entity");
		});
		...
		$scope.loadPage = function (pageNumber, filter) {
			...
			entityApi.count(filter).then(function (response) {
				...
				request.then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("Person", `Unable to list/filter Person: '${response.message}'`);
						return;
					}
